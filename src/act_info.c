...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
	send_to_char_bw( victim->description, ch );
    }
    else
    {
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"You are %s%s{x, level %d, %d years old (%d hours).\n\r",
	ch->name,
    ...
    {
	sprintf( buf, "You need %d experience points to level.\n\r",
...
void do_who( CHAR_DATA *ch, char *argument )
{
	 ...
	 * Format it up.
	 */
	sprintf( buf + strlen(buf), "[%2d %s] %s %s{x\n\r",
	    wch->level,
	    class,
	    wch->name,
	    wch->pcdata->title );
...
void do_description( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char_bw( ch->description ? ch->description : "(None).\n\r", ch );
    return;
}
...
void do_password( CHAR_DATA *ch, char *argument )
{
    ...
    *pArg = '\0';
    ...
    {
	WAIT_STATE( ch, 20 );
	send_to_char( "Wrong password.  Wait 5 seconds.\n\r", ch );
	return;
    }
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(  IS_SET(ch->act, PLR_COLOR)
	    ? "[+ANSI     ] Your ansi color is on.\n\r"
	    : "[-ansi     ] Your ansi color is off.\n\r"
	    , ch );
	...
	else
	{
	    ...
	    return;
	     ...
	     if ( !str_cmp( arg+1, "ansi"     ) ) bit = PLR_COLOR;
