...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	send_to_char( "You may not attack a upper level player.\n\r", ch );
	return TRUE;
    }
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	sprintf( buf2, "You {g%s{x $N%c", vs, punct );
	...
	sprintf( buf2, "Your %s {g%s{x $N%c",  attack, vp, punct );
	sprintf( buf3, "$n's %s {r%s{x you%c", attack, vp, punct );
    }
