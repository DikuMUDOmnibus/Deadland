...
void move_char( CHAR_DATA *ch, int door )
{
	...
	||   to_room->sector_type == SECT_WATER_NOSWIM )
	{
	    ...
	    if ( IS_IMMORTAL(ch) || IS_AFFECTED( ch, AFF_FLYING ) )
		found = TRUE;
    ...
    if ( ( ch->hit < (ch->max_hit * 2) / 10 ) && !IS_NPC(ch)
    &&	   !IS_IMMORTAL(ch) )
    {
	send_to_char( "A blood trickles from your wounds.\n\r", ch );
	act( "$n leaves a trial of blood.", ch, NULL, NULL, TO_ROOM );
	if ( ( obj = get_obj_list( ch, "blood", ch->in_room->contents ) )
	    == NULL )
	{
	    obj = NULL;
    ...
    if ( !IS_AFFECTED(ch, AFF_SNEAK)
    && ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) ) )
	act( "$n has arrived from $T.", ch, NULL, dir_rev[door], TO_ROOM );
