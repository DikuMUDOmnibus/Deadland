...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	{
	    victim    = ch->fighting;
	    condition = victim->hit * 100 / victim->max_hit;

		 if ( condition < 25 )	 strcpy( condbuf, "awful"   );
	    else if ( condition < 50 )	 strcpy( condbuf, "poor"    );
	    else if ( condition < 75 )	 strcpy( condbuf, "fair"    );
	    else if ( condition < 100 )	 strcpy( condbuf, "good"    );
	    else if ( condition >= 100 ) strcpy( condbuf, "perfect" );

	    sprintf( buf, "<[%s] [%dhp %dm %dmv]> ", condbuf, ch->hit,
		ch->mana, ch->move );
	}
	else
	    sprintf( buf, "<[%d] [%dhp %dm %dmv]> ", 
		(ch->level + 1) * 1000 - ch->exp, ch->hit, ch->mana, ch->move );
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    case CON_READ_MOTD:
	write_to_buffer( d, 
    "\n\rWelcome to Deadland.  May your visit here be ... Mercenary.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
...
bool check_parse_name( char *name )
{
     ...
     * Reserved words.
     */
    if ( is_name( name, "all auto immortal self someone deadland diku merc" ) )
	return FALSE;
